...
<div class="container mt-4">
    ...
    <form action="{{ route('edit_chitiet', $chitiet->id_chitiet) }}" method="POST" enctype="multipart/form-data" class="row g-3">
        @csrf
        @method('PUT')
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            ...
            <select name="tinhtrang_sanpham" class="form-select">
                <option value="Liên hệ" {{ $chitiet->tinhtrang_sanpham == 'Liên hệ' ? 'selected' : '' }}>Liên hệ</option>
                <option value="Còn hàng" {{ $chitiet->tinhtrang_sanpham == 'Còn hàng' ? 'selected' : '' }}>Còn hàng</option>
            </select>
        </div>
        ...
        @for ($i = 1; $i <= 4; $i++)
            <div class="col-md-6">
                ...
                <input type="file" name="anhchitiet{{ $i }}" value="{{ $chitiet->{'anhchitiet'.$i} }}" class="form-control">
            </div>
        @endfor
